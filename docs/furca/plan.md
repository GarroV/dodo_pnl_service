...
Канон проверок — этот раздел. Приёмка блока обязана звать **объявленную** здесь
команду, а не собранную на месте: собранная расходится с тем, что гоняет CI, и
«зелено у меня» перестаёт что-либо значить.

Источник этих команд — `.github/workflows/ci.yml`; если правится он, правится и
таблица, тем же изменением.

| Проверка | Команда | Что ловит |
|---|---|---|
| Линт | `ruff check .` | стиль и мёртвые импорты. Держал master красным с 30.08 по 02.09 на шести длинных строках |
...
приёмка читает **состав** прогона, а не только код возврата: сколько проверок
выполнено, сколько пропущено и с какой причиной. Это не педантизм — ноль
выполненных проверок и две с половиной тысячи зелёных дают на выходе одинаковый
ноль. На этом проекте оба конца уже случались: команда `python -m pytest` вернула
`0` со словами `command not found`, а обёртка вокруг объявления теста убирает
проверки из отчёта совсем, оставляя прогон зелёным.

**Живой Postgres обязателен.** Без базы тесты схемы и доступа не падают, а
**пропускаются** — и прогон зеленеет, не проверив разграничение доступа вовсе.
Именно так когда-то прожил незамеченным дефект видимости регистров.

**Тесты доступа гоняются ролью `app_user`** (`as_app_user` в `conftest.py`):
владелец таблиц и суперпользователь обходят RLS, поэтому политику можно написать
неправильно и получить зелёный прогон.

**Рабочий процесс очереди при прогоне не поднимается.** Живая очередь
подхватывает задания, заведённые тестами, и пересчитывает данные у теста под
руками (issue #65).

**Порог покрытия — относительный, а не абсолютный:** ниже, чем на прошлой
приёмке, — блок не принят. Абсолютная цифра остаётся ориентиром и сама по себе
блок не заворачивает: она подталкивает дописывать тесты без утверждений ради
процента, а такой тест приёмка засчитает выполненной проверкой.
