...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "web.i18n.switcher",
                # Состояние экрана: готов, дорабатывается или ещё не написан
                # (D064). В каркасе, потому что пометку, которую надо не забыть
                # дописать в каждый из сорока девяти шаблонов, однажды забудут.
                "web.stages.current",
            ],
        },
    },
...
# забирает задачу примерно за половину цикла опроса (PAYRUN_QUEUE_POLL_SECONDS
# ниже), поэтому порог держится кратно больше — связь проверяется тут же.
