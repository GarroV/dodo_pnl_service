...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                # Разделы шапки — из одного словаря (issue #162): шапка стоит
                # на всех страницах, и собирать её в каждом представлении
                # значило бы столько же мест, где о разделе можно забыть.
                "web.navigation.sections",
                # Тема ставится сервером в атрибут <html>, поэтому её нужно
                # знать при рендере каждой страницы (issue #164).
                "web.theme.theme",
