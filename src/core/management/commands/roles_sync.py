...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        if not report.bypasses_rls:
            # Не решение, а честность: если роль базы не видит часть строк,
            # отчёт выше говорит не «доставлять нечего», а «отсюда не видно».
            self.stdout.write(
                "Внимание: роль базы, которой шла проверка, не обходит RLS — "
                "часть ролей могла остаться невидимой, и отчёт выше это не "
                "доказывает."
            )
