...
ROLES = [
    SeedRole(
        ...
        # `unit.close` — закрытие часов точки (D033, T076): у управляющего оно
        # тоже есть, но только на свою точку, а у директора точек не задано,
        # то есть все. Без этого закрыть месяц было некому, пока управляющий в
        # отпуске, — при том что весь период директор утверждает.
        [
    ...
    SeedRole(
        ...
        # Все три регистра, как у директора (D036, ответ владельца на Q012).
        # Раньше здесь стоял один официальный — догадка, что «официальная роль
        # не должна видеть остальное». У партнёра это сегодня не так: бухгалтер
        # и оперативный директор равны. Догадка стоила дорого: расчёт периода —
        # то, что спека требует от бухгалтера прямо, — не проходил вовсе (база
        # отвергает запись строк регистров, которых роль не видит), а выгрузка
        # P&L молча приходила без налогов и взносов.
        #
        # Механизм видимости при этом не разобран: он на месте и работает —
        # у управляющего точки набор по-прежнему неполный (D031), и политики
        # режут ему внутренний регистр. Сужать доступы будем позже, там где
        # надо, — и механизм к тому времени должен быть жив.
        ["official", "supplementary", "internal"], None,
        # Набор прав равен директорскому (D036, T115). `period.reopen` и
        # `unit.close` пришли последними: без них единственный человек, который
        # в сценарии закрытия месяца доходит до утверждения, не мог исправить
        # собственную опечатку — за откатом надо было идти к директору, при том
        # что спека требует откат от бухгалтера дословно. Живой базе то же самое
        # приезжает миграцией `0230`: сид там не гоняется.
        [
            "timesheet.edit", "payrun.calculate", "period.approve",
            "period.reopen", "payslip.freeze", "retro.post", "unit.close",
        ],
    ...
    SeedRole(
        ...
        # Все три регистра, а не один официальный (T089, тот же довод, что в
        # D033). Справочники и правила ведёт он и только он: `directory.manage`
        # и `rules.manage` больше нет ни у кого. С одним официальным регистром
        # это давало тупик, а не разграничение: карточки курьеров и кухни он не
        # видел вовсе — админка показывала 8 сотрудников из 35 и 3 группы из 6, —
        # то есть **ни один пользователь продукта** не мог поменять курьеру
        # ставку или закрыть группу. Хуже того, число «8» подавалось без
        # оговорок: узнать, что есть ещё 27 карточек, было неоткуда.
        #
        ...
        # вправе выписать себе недостающий регистр. Ограничение было не защитой,
        # а лишним шагом на пути к тому же результату.
        #
