...
Суперпользователь обходит RLS всегда, включая force. Это осознанно, но касается
только администрирования данных: сид, восстановление из дампа, обслуживание.
**Работа приложения от чьего-либо права обходить RLS не зависит** — см. ниже
про memberships (T052).
...
4. **`memberships` — единственная таблица, чья политика не зовёт функций
   контекста** (T052, issue #44). На ней держится всё остальное: `app_tenant_ids()`
   читает именно её. Пока политика этой таблицы сама была «тенант из
   `app_tenant_ids()`», получался круг — политика зовёт функцию, функции нужна
   та же политика. С владельцем-суперпользователем круг не замыкался: он
   обходит RLS, и внутри функции политика не применялась. С обычным владельцем
   (а на управляемом Postgres другого не бывает — `bypassrls` выдаёт только
   суперпользователь) тот же запрос уходил в рекурсию и падал с `stack depth
   limit exceeded`. Поэтому здесь условие прямое: `user_id = app_user_id()` —
   свои членства и ничьи больше. Оно ничего не читает и замкнуться не может.
   Побочно это сужает видимость: человек больше не видит членства коллег по
   тенанту. Так и надо — кто ещё работает у партнёра, из ведомости не следует;
   когда понадобится экран управления людьми, он получит данные функцией
   `security definer`, а не расширением этой политики.
"""
...
# Таблица, из которой функции контекста берут ответ. Её политика обязана быть
# самодостаточной: любое обращение к функциям отсюда — рекурсия (пункт 4 в шапке).
ROOT_TABLE = "memberships"
...
def _tenant_policies() -> str:
    ...
    for table in TENANT_TABLES:
        if table == ROOT_TABLE:
            # Корень контекста: условие прямое, без вызова функций. Иначе
            # политика зовёт app_tenant_ids(), а той нужна эта же политика —
            # см. пункт 4 в шапке модуля.
            parts.append(f"""
