...
подразумевается из других: роль без него база не пускает — как и интерфейс,
который в этом случае не показывает кнопки.

Кому право выдано, решает не эта миграция, а набор прав роли. Здесь оно было
только у управляющего; миграция `0110` добавила его оперативному директору
(D033, T076) — иначе месяц нельзя было закрыть, пока управляющий в отпуске.
Политики при этом не менялись ни одной: срез «управляющий закрывает только
свою точку» держит `unit_visibility` ниже, а у директора точки не заданы вовсе.
