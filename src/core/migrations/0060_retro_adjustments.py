...
- Не запрещает править входные данные закрытого месяца. Запрет решил бы задачу
  ...
  дельту, а не отказ.
- Не отменяет переносов удалением: отмена — пометка, и ставит её только триггер.
  Удалить строку можно, лишь пока разница не утверждена у получателя; после
  утверждения запись о выплаченной разнице не стирается ничем.
"""
...
begin
    if tg_op = 'DELETE' then
        ...
        -- Удаление запрещено ровно тогда, когда разница уже утверждена у
        -- получателя: она выплачена, и стереть запись о выплате нельзя.
        -- Полного запрета здесь намеренно нет. Он выглядел бы строже, а на деле
        -- воспроизвёл бы дефект, который в этом блоке ловили уже дважды
        -- (issue #60 и #62): обслуживание и сид не смогли бы убрать за собой
        -- вообще ничем, и первый же перенос делал бы базу неперезаливаемой.
        if exists (
            select 1 from payruns p
             where p.tenant_id = old.tenant_id
               and p.period = old.target_period
               and p.status = 'approved'
        ) then
            raise exception 'разница уже утверждена у получателя: удалить перенос нельзя'
                using hint = 'откройте период-получатель заново, если запись действительно надо убрать';
        end if;
        return old;
    end if;
...
comment on function retro_adjustment_guard() is
    'Перенос разницы: правится только отмена; удаляется, пока разница не утверждена у получателя';
