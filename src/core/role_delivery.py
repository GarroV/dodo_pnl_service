...
нет — **первой строкой отчёта и вместо** вывода «роли совпадают с кодом», а не
оговоркой после него; а после записи перечитывает роль и падает, если запись не
легла.
"""
...
# Отчёт, снятый ролью, которая не обходит политики, не отвечает на вопрос, ради
# которого его смотрят. Текст один на все поверхности: и `migrate`, и команда
# говорят это одними словами, иначе две формулировки разъедутся на первой правке.
_BLIND = (
    "сверка ничего не доказывает: роль базы не обходит RLS, и сколько ролей на "
    "самом деле в таблице, отсюда не видно. Доставку выполняет владелец схемы "
    "(MIGRATION_DB_USER)."
)
...
def sync(connection, *, apply: bool = True, adopt: bool = False, say=None) -> Report:
    ...
    with connection.cursor() as cursor:
        ...
        # значит «не видно», а не «нечего делать». Словами об этом говорит
        # `describe()` — там же, где остальной отчёт, и вместо вывода про
        # согласие, а не после него (см. `_BLIND` и приёмку на живом стенде).
...
def describe(report: Report) -> list[str]:
    ...
    if not report.bypasses_rls:
        # ПЕРВОЙ строкой и вместо вывода про согласие, а не после него. Найдено
        # приёмкой на живом стенде: отчёт начинался с «роли совпадают с кодом
        # (0 шт.)» и лишь потом оговаривался. Первая строка — то, что человек
        # запоминает, и она была неправдой ровно в том случае, ради которого
        # проверка обхода политик и заведена.
        lines.append(_BLIND)
    # Принятие идёт раньше доставки и в отчёте: `--adopt` сначала объявляет
    # состояние поставленным, и только после этого роль становится отставшей и
    # её довозят. Обратный порядок читался бы как «довезли, а потом зачем-то
    # приняли».
    ...
    if not lines:
        # «Совпадают» говорится только там, где это можно утверждать: под
        # политиками ноль расхождений означает ноль видимых строк.
