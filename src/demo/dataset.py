...
class Person:
    ...
    # Чем меряется работа. Пусто — как у группы (часы). Оклад показывается
    # демо намеренно: вторая форма оплаты рядом с почасовой (issue #188), и
    # без живого окладника её в продукте не видно ни на одном экране.
...
PEOPLE: list[Person] = [
    ...
    # Двое на окладе: сумма за месяц вместо цены часа. Их часы в табеле стоят
    # обычные — оклад делится на норму месяца, поэтому полная норма даёт ровно
    # оклад, а неполный месяц — пропорцию.
    Person("Olivia", "Bennett", "office", "BG1", D("172000.00"), D("1.00"),
           work_measure="salary"),
    Person("Daniel", "Foster", "office", "BG1", D("148000.00"), D("1.00"),
           work_measure="salary"),
