...
from .seed import ROLES, role_password
...
def enter(request, role: str = DEFAULT_ROLE):
    ...
    # Пароль спрашивается у сида: у администратора сети он свой (D052 —
    # его учёткой смотрят продукт целиком, и она входит парой admin/admin).
    user = login_with_password(request, role, role_password(role))
    if user is None:
...
def accountant_file(request):
    ...
    **Файл один и тот же для всех, кто его открыл** (T104, issue #88). Это
    ...
    if not request.user.is_authenticated:
        if login_with_password(request, DEFAULT_ROLE, role_password(DEFAULT_ROLE)) is None:
