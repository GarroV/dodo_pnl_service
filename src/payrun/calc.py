...
def collect_cases(tenant_id: UUID, period: date) -> list[Case]:
    ...
    if missing:
        ...
        # срезом он рассказывает о том, чего ей не показывают: столько же
        # утечки, сколько в имени. У того, кому список отдан, он и так стоит
        # рядом — и там его видно точнее, чем числом.
        raise PayrunRefused(
            _("нет условий найма на этот месяц. "
...
def check_insured_base(cases: list[Case], rules) -> None:
    ...
    if stale:
        # Без счёта людей — по тому же доводу, что в `build_cases` выше (T114).
        raise PayrunRefused(
            _("база для взносов не сходится с часами табеля. "
