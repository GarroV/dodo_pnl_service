...
**Своя граница видимости не выдаётся за факт о базе.** Строка файла, которой в
отданном роли срезе нет, называется не отсутствующей, а не входящей в этот срез
(T095). Разница не словесная: «в расчёте такой строки нет» — утверждение о базе,
которого сверка не проверяла и проверить не может, а управляющий одной точки
читал его про всех, кто работает на других.

**Обратный список — только тому, кому отдан весь расчёт** (T100). «Есть в
расчёте, нет в таблице» — утверждение о **расчёте**, и роль, которой расчёта не
показали, проверить его не может. Хуже: наша сторона собирается из двух выборок
с разной видимостью — входы (табель, условия найма) роль видит по обычным
политикам, итоги нет, — и разность «наша сторона минус файл» превращалась в
поимённый список тех, чьи выплаты лежат в невидимом роли регистре. Не
вычитанием, а перечислением. Поэтому список строится при двух условиях сразу:
роли отдан весь расчёт (свойство роли, `whole_run_visible`) **и** по самой
строке итоги есть (иначе «есть в расчёте» неправда и у директора).

Отсюда правило пошире, которым стоит мерить каждый следующий раздел этого
экрана: **имя на сверке берётся из файла, который человек принёс сам.** У файла
границы видимости нет — он его и загрузил. Всё, что сверка добавляет от себя,
приходит из базы, то есть уже разрезано ролью, и любая **разность** двух
по-разному разрезанных выборок называет людей. Именно так вышло здесь: сама база
ничего лишнего не отдавала, список изготовило приложение, сложив две выборки.
...
class Reconciliation:
    ...
    # Отдан ли роли расчёт целиком. Не украшение ответа: на этом стоит, вправе
    # ли сверка вообще говорить что-либо **о расчёте** — например, кого в нём
    # не хватает по сравнению с файлом. Показывающий обязан спросить это поле, а
    # не догадываться по пустому списку: пустой список и «сказать нечего» —
    # разные вещи, и второе надо произнести вслух.
    whole_run_visible: bool = False
...
def compare(
    ...
    *,
    ...
    `whole_run_visible` — отдан ли роли расчёт целиком (все регистры учёта).
    Умолчание **закрытое** намеренно: забывший его вызов не назовёт ни одного
    человека. Ошибаться здесь можно только в сторону «сказали меньше»: обратная
    ошибка — это поимённый список людей скрытого регистра (T100).
    """
    result = Reconciliation(
        findings=list(findings or []), whole_run_visible=whole_run_visible
    )
    seen: set[str] = set()
    ...
    for row in rows:
        ...
        ours = run.get(key)
        if ours is None:
            # Ни числа, ни разницы. Ответ базы одинаков для того, кого в
            # расчёте действительно нет, и для того, чью строку роли не видно:
            # различить их снаружи нельзя намеренно. Значит и **сказать**
            # «такой строки нет» нельзя — это утверждение о базе, которого
            # сверка не проверяла (T095). У управляющего одной точки так
            # выглядели все остальные точки сразу: шестнадцать строк, о которых
            # продукт сообщал, что бухгалтер принёс лишних людей.
            #
            # Названо ровно то, что проверено: строка не входит в срез, который
            # база отдала этой роли. Где она лежит на самом деле — не говорится
            # ни словом: это и был бы D023.
            result.only_in_file.append(Absent(
                key, row.name, row.sheet,
                _("не входит в доступную вам часть расчёта этого периода"),
            ))
            continue
    ...
    # Обратная сторона — только тому, кому база отдала весь расчёт (T100).
    # Условие проверяется здесь, а не в цикле, потому что оно про роль, а не про
    # строку: пропусти мы «только скрытые» строки, оставшийся список отличался бы
    # у ролей с разным набором регистров — и разница снова называла бы людей.
    if whole_run_visible:
        for key, ours in run.items():
            if key in seen:
                continue
            if not ours.totals:
                # Итогов по строке нет вовсе — значит расчёта по ней не было, а
                # был только табель. «Есть в расчёте» про неё неправда даже у
                # директора, а у роли с неполным набором регистров именно такие
                # строки и составляли весь список.
                continue
            # Отрицание здесь прямое, и это не оговорка: файл человек загрузил
            # сам и видит его целиком, границы видимости у файла нет. Смягчать
            # проверенный факт заодно с T095 значило бы его потерять.
...
def whole_run_visible(tenant_id: UUID) -> bool:
    """Отдан ли этой роли расчёт целиком — то есть видны ли ей все регистры.

    Спрашивается **та самая функция, на которой стоят политики**
    ...
    Тот же вопрос и тем же способом задаёт заморозка строки
    (`payrun.freezing.rows_are_freezable`, T101) — общий у них не код, а
    источник ответа.

    Почему не «есть ли хоть один отданный итог». Пустой ответ базы одинаков у
    роли со скрытым регистром и у периода, который ещё не считали: догадка по
    пустоте объявила бы директору, что расчёт ему не отдан, ровно там, где
    расчёта просто нет. А ошибка в другую сторону — раздел с именами.
    """
