...
def compare(
    ...
    *,
    ...
    for key, ours in run.items():
        if key not in seen:
            # Обратная сторона отрицает прямо, и это не оговорка: файл человек
            # загрузил сам и видит его целиком, границы видимости у файла нет.
            # Смягчать проверенный факт заодно с T095 значило бы его потерять.
