...
(function () {
  ...
  window.addEventListener("pagehide", function () {
    ...
    cells().forEach(function (input) {
      ...
      // летит, либо его не было вовсе (уход прямо из поля).
      //
      // Одно и то же значение досылается ОДИН раз. `pagehide` срабатывает не
      // однажды: уход со страницы бывает двойным (страница входа, затем форма
      // входа), и тогда с одной ячейки уходили две одинаковые досылки сразу —
      // две записи в одну строку табеля в одну секунду (issue #61). Сервер
      // теперь их переживает (строка берётся под блокировку), но второй запрос
      // не нужен и без того: он ничего не добавляет, а ответа его никто не
      // читает. Пометка — само отправленное значение, а не «отправляли»: если
      // человек успел набрать другое, оно уедет.
      if (input.dataset.beaconValue === input.value) return;
      input.dataset.beaconValue = input.value;
      //
