...
def save_rule(who, item, *, ledger: str, method: str, unit_id, valid_from: date) -> bool:
    ...
    what = _("правило разнесения статьи расходов")
    directory.refuse_if_touches_closed_month(who.tenant_id, valid_from, what)
...
def _close(who, current, valid_from: date) -> bool:
    ...
    directory.refuse_if_touches_closed_month(
        who.tenant_id, valid_from, _("правило разнесения статьи расходов"),
    )
    if valid_from <= current.valid_from:
        raise BadInput(
            _(
