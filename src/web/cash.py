...
class UnitRefused(Exception):
    ...
    def __init__(self, message: str | None = None):
        # Своя формулировка нужна ровно одному случаю — расходу на всю сеть
        # (`NETWORK_REFUSED`). Он не точка из справочника, а известный всем
        # вариант формы: прятать его существование не от кого, и «не найдена»
        # оставила бы человека гадать, что он сделал не так. D023 про точки —
        # чтобы перебором нельзя было составить список чужих, — а не про этот
        # вариант.
        self.message = message or _("Точка не найдена.")
...
def _refusal(payload: dict) -> UnitRefused:
    ...
    его вносит тот, кто ведёт все точки партнёра (T130). Общая формулировка
