...
def expense_item(request, item_id=None):
    ...
    if request.method == "POST":
        try:
            ...
            item.save()
            return redirect(_after_save(request, who, item))
...
def _rule_notice(request) -> str:
    """Итог пересчёта после правки правила — теми же словами, что на экране
    нераспределённого: один и тот же пересчёт не должен объясняться по-разному."""
    from .expenses_views import _spread_notice

    return _spread_notice(request)
