...
модели и решений — в `core.models.ExpenseItem` и `docs/furca/...`; здесь только
...
def _list_page(request, who, *, error: str = "", status: int = 200):
    ...
    return render(request, "web/directory/list.html", {
        ...
        # Названия колонок перечисляются на всех трёх языках — теми самыми
        # написаниями, которые принимает разбор (`COLUMNS` в
        # `web/expense_items_upload.py`). Иначе перевод даёт неверный совет:
        # сербская страница называла русские заголовки, и бухгалтер Сербии, ради
        # чьего файла заведена вся загрузка, переименовывала бы рабочие
        # `Naziv`/`Šifra` в кириллицу без всякой нужды (T157, находка Н8). Тот
        # же приём уже применён в тексте отказа по файлу — здесь он доведён до
        # второй строки из двух.
        "upload_about": _(
            "Список статей бухгалтера книгой Excel. Колонки продукт ищет по "
            "названию, на любом из трёх языков: «Название», «Naziv» или «Name»; "
            "«Код», «Šifra» или «Code»; «Строка P&L», «Kategorija» или «P&L»; "
            "«Действует с», «Važi od» или «Valid from». Порядок и лишние колонки "
            "значения не имеют. Дата из файла главнее поля ниже: оно применяется "
            "только к строкам, где даты нет, а непонятную дату продукт назовёт и "
            "строку пропустит. Повторная загрузка того же файла ничего не удвоит: "
            "статьи сходятся по коду. Строки справочника, которых в файле нет, "
            "останутся — их судьбу решаете вы."
        ),
        "upload_lines": [
