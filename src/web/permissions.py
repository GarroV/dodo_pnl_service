...
# Ведение справочников: сотрудники, условия найма, группы, точки, юрлица,
# производственный календарь (T018). Отдельное право, а не следствие «умею
# считать»: у администратора сети в сиде нет ни одного права на данные расчёта,
# и это намеренно — справочники ведёт он, а суммы не трогает вовсе.
DIRECTORY_MANAGE = "directory.manage"
# Ведение правил расчёта: ставки, проценты, схемы, чем меряется работа группы
# (T090). Отдельное от `directory.manage`, хотя в сиде оба у одного человека:
# справочник отвечает на «кто у нас работает», правила — на «как из этого
# получаются деньги», и партнёр вправе развести их по разным людям. Своим
...
# Разбор первички: назначить принесённой бумаге или строке без статьи статью
# расхода, период и поставщика — то есть превратить её в строку P&L (T174).
#
# Право появилось потому, что его отсутствие нарушало обещание задачи. Роль
# управляющего точки — донести бумагу, а не посчитать её; но карточка бумаги
# показывала ему форму разбора и кнопку «Разобрать и учесть», и нажатие ставило
# сумму в отчёт немедленно. Ни одной проверки на пути не было: кто пишет факт,
# решала только видимость строки политиками базы, а она знает про точку и
# регистр, но не про то, кому положено классифицировать.
#
# Осторожно с широтой: право касается ТОЛЬКО разбора. Наличный расход своей
# точки управляющий вносит по-прежнему (T109) — там он не классифицирует чужую
# бумагу, а вносит свою трату, и статью выбирает сам вносящий.
SUPPLIERS_CLASSIFY = "suppliers.classify"
...
TITLES = {
    ...
    SUPPLIERS_CLASSIFY: gettext_noop("Разбор первички"),
}
