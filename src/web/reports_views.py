...
def period_export(request, period_id, kind):
    ...
    book, name = build(
        ...
        # Разрез по НДС (T146, D042). Умолчание — суммы без налога: «в итоговом
        # ПНЛ мы обычно не показываем НДС». Значение читается как есть и
        # сверяется со списком известных — чужое слово в адресе не должно
        # оборачиваться пятисоткой и не должно молча менять числа в файле.
        vat=exports.GROSS if request.GET.get("vat") == exports.GROSS else exports.NET,
    )
