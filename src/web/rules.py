...
**Правило третье: закрытый месяц не правится ничем.** Проверка — та же самая
функция, что у справочников (`web/directory.refuse_if_touches_closed_month`), а
не своя копия: два правила «не задевай закрытый месяц» разъехались бы на первой
правке, и одно из них однажды пропустило бы то, что второе ловит.
...
from .directory import refuse_if_touches_closed_month
...
def save_override(tenant_id, path: str, value: Any, *, valid_from: date,
    ...
    refuse_if_touches_closed_month(tenant_id, valid_from, _("правило расчёта"))
