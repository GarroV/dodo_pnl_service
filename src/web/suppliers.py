...
def document_or_none(document_id):
    """Счёт по номеру — под политиками базы, и только своего вида.

    Документ другого вида (чек, строка выписки) сюда не попадает: править его
    экраном счетов нечем, а 404 на него — тот же ответ, что на чужой (D023).
    """
    return SourceDocument.objects.filter(
        pk=document_id, kind=INVOICE_KIND, source=MANUAL_SOURCE,
