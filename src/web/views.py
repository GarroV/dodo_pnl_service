...
# Демо спрашивают ровно об одном: показывать ли путь внутрь (issue #116).
...
def index(request):
    ...
    Issue #116: на стенде корень уводил на `/periods/`, оттуда `@login_required`
    на форму входа, а форма предлагала пароль от продукта, которого у гостя
    нет. Посетитель, потерявший `?key=…` в ссылке, оказывался в тупике, который
...
def login_context(request) -> dict:
    ...
    return {
        ...
        # (issue #116). Не «включено ли демо»: со включённым ключом гость без
        # ключа получил бы по этой кнопке `404`, то есть форма входа обещала
        # бы вход и не пускала.
