...
def person_row(html: str, name: str) -> str:
    """Строка человека на экране ролей — а не первая строка с таким текстом.

    Жил в `test_roles_screen_access.py`, переехал сюда 03.09.2026, когда тот же
    дефект второй раз выстрелил в соседнем файле — и выстрелил так, что виден
    был только на другой операционной системе.

    Ловушка в том, что название роли и имя человека на этом экране совпадают:
    в сиде бухгалтера зовут «Бухгалтер». Выше по странице стоит таблица ПРАВ, где
    то же слово написано в ячейке роли, поэтому поиск «первой ячейки с этим
    текстом» попадает в неё, а следующая форма `/roles/people/` принадлежит уже
    **первому человеку списка**, кто бы он ни был.

    Дальше всё решает сортировка людей, а она зависит от коллации базы: на macOS
    первым идёт «Бухгалтер» и тест случайно попадает в нужного, на glibc —
    «Администратор сети», и роль администратора выдаётся администратору. Тест,
    который зависит от того, что встретится раньше, врёт молча — и врёт по-разному
    на разных площадках.
    """
    rows = [
    ...
    for row in rows:
        # Сравниваем ПЕРВУЮ ячейку — имя человека, а не «строка содержит это
        # слово». В той же строке стоит список выдачи, где перечислены названия
        # всех ролей продукта, поэтому «Бухгалтер» встречается в строке каждого
        # человека. Проверка на вхождение находила первого в списке — и на
        # разных площадках это были разные люди.
        first_cell = re.search(r"<td\b[^>]*>(.*?)</td>", row, flags=re.S)
        if first_cell and name in re.sub(r"<[^>]+>", " ", first_cell.group(1)):
            return row
    raise AssertionError(f"человека «{name}» нет в списке людей ({len(rows)} строк)")


def content(response) -> str:
    """Только содержимое экрана — без шапки, языка и каркаса.

    Сюда обязана смотреть любая проверка вида «на экране НЕТ такого-то». В
    шапке стоят названия всех разделов продукта, и поиск подстроки по целому
    документу ловит их вместо содержимого.

    Куплено дважды за один заход (D064, шапка получила все пункты эталона):
    проверка «экран сотрудников не отдаёт расчёт» нашла слово «Ведомость» в
    пункте меню, а проверка «сверка не называет скрытого человека» нашла имя
    «Ана» внутри «Аналитики по людям». Оба раза продукт был прав, а тест — нет,
    и оба раза это выглядело как настоящая утечка.
    """
    import re

    html = body(response)
    inside = re.search(r"<main[^>]*>(.*)</main>", html, re.S)
    return inside.group(1) if inside else html
