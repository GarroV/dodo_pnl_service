...
def test_the_header_says_whose_eyes_these_numbers_are(client, web_env):
    ...
    # Регистры названы метками своего цвета, а не строкой «регистры: A, B, C»
    # (раздел «Семантика регистров» эталона). Проверяется смысл, а не приём:
    # у роли на экране стоят названия ровно тех регистров, которые ей видны,
    # и каждое своим классом — иначе метки снова стали бы одинаково серыми.
    assert "Официальный" in page, "не сказано, что видно"
    assert "reg--official" in page, "регистр показан, но не своим цветом"
