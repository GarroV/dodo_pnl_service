...
# Управляющий видит два регистра — официальный и дополнительный (D031, ответ
# владельца на Q010): он отвечает за свою точку и обязан видеть надбавки своей
# смены. После D036 это ЕДИНСТВЕННАЯ роль с неполным набором, и на ней стоят
# все проверки видимости регистров.
ROLE_LEDGERS = {
    "director": {"official", "supplementary", "internal"},
    # Все три, как у директора (D036, ответ владельца на Q012): бухгалтер и
    # оперативный директор равны. Один официальный был догадкой стройки, и она
    # ломала работу, которую спека требует от бухгалтера прямо, — расчёт
    # периода.
    "accountant": {"official", "supplementary", "internal"},
