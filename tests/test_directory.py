...
# Роли без права вести справочники. Осталась одна — управляющий точки.
#
# Владелец 28.08.2026: «операционный директор мне кажется может вести
# справочники, почему нет?» (D059). Бухгалтер получил право вместе с ним, и это
# не расширение ответа: D036 говорит, что у партнёра эти две роли равны по
# доступу, а дать право одному и не дать другому значило бы молча отменить
# прежнее решение владельца. У партнёра именно бухгалтер ведёт проект целиком.
#
# Правила страны и роли остались администратору сети: это платформенный
# уровень, а не работа партнёра.
ROLES_WITHOUT_RIGHT = ["manager"]

# Справочники, закрытые правом целиком.
#
# Сотрудников здесь больше нет, и это не смена вкуса. Раньше в списке стоял
# `/directory/employees/`, то есть проверка ниже требовала, чтобы управляющему
# точки экран сотрудников отвечал отказом. Требование было **неверным**: ставки
# живут только у нас (в Dodo IS их нет вовсе — это условия найма), а проверять
# ставки, должности и имена своих людей — прямая работа управляющего. Строка
# закрепляла не правило, а дефект: единственный экран, где нужные ему данные
# существуют, отказывал именно ему. Разобрано в D047, починено в T173.
#
# Правильная граница — та, что уже принята в продукте для контрагентов: строки
# режет база, а право решает «правка или чтение», а не «видно или нет». Поэтому
# в списке остались справочники, у которых читателя, которому они нужны для
# работы, нет: у них право по-прежнему закрывает экран целиком.
#
# Гарантия, которую несла убранная строка, не потеряна, а переехала: то, что
# чтение сотрудников не открыло управляющему остальные справочники, стережёт
# `test_reading_people_did_not_open_the_rest_of_the_admin` в
# `tests/test_manager_reads_employees.py` — там же и весь разбор экрана.
DIRECTORY_URLS = [
    "/directory/",
    "/directory/groups/",
    "/directory/units/",
...
def test_only_the_role_that_manages_directories_sees_the_link(client):
    """Ссылка на справочники есть у тех, кто их ведёт, и только у них.

    Директор и бухгалтер добавились решением владельца 28.08.2026 (D059 + D036).
    Управляющий точки справочники не ведёт — ему ссылка не показывается, и это
    не оформление: кнопка на отказ хуже отсутствующей.
    """
    seen = {}
    ...
    assert seen == {"director": True, "accountant": True, "manager": False, "admin": True}
