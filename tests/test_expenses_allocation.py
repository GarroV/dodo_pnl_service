...
def test_a_retroactive_rule_is_refused_and_the_closed_month_stays(
    client, sql, web_env, item, units, rules_removed, facts_removed,  # noqa: F811
    payruns_restored,  # noqa: F811
):
    """Правило задним числом в утверждённый месяц не заводится (D020).

    Расход вносится и разносится в открытом июне, июнь закрывается — и попытка
    задать правило с июньской датой отвергается словами. Закрытый месяц при
    этом не двигается ни на копейку: это и проверяется числом до и после.
    """
    login_as(client, "admin")
    set_rule(client, item, method="even")
    client.post("/logout/")
    ...
    try:
        ...
        assert "утверждена" in body(answer), body(answer)[:2000]
        ...
        ).fetchone()[0] == 1, "правило задним числом всё-таки завелось"
    finally:
        client.post("/logout/")
