...
def test_the_manager_sees_no_network_expense_at_all(
    client, sql, item, units, rules_removed, facts_removed,  # noqa: F811
):
    """Управляющему на этом экране не обещают ничего, потому что и показывать нечего.

    Проверка написана, когда управляющий сетевые расходы ВИДЕЛ, и защищала его
    от обещания кнопки, которая ему откажет: причина считалась по плану
    разнесения, а план у него заведомо короче — `allocation_plan` читает `units`
    под политиками вызвавшего.

    Параллельно шла другая починка (T130): у «строки без точки» появился новый
    смысл — трата юрлица целиком, — и роль с урезанным набором точек её больше
    не видит и завести не может. То есть исходная развилка исчезла вместе с
    доступом, и проверять теперь надо не формулировку, а сам факт: сетевого
    расхода у управляющего нет ни строкой, ни обещанием.

    Переписана, а не удалена, намеренно: удалив её, мы остались бы без проверки
    на то, что экран ожидания не выдаёт роли чужие сетевые траты — а это ровно
    тот класс, из-за которого задача и заводилась.
    """
    login_as(client, "admin")
    ...
    try:
        refused = client.post(
            NEW,
            {
                "date": august_day(), "amount": "700.00", "item": item,
                "note": "аренда офиса", "unit": NETWORK, "ledger": "official",
                "entry_key": entry_key(),
            },
        )
        assert refused.status_code == 400, (
            f"управляющий завёл расход на всю сеть: {refused.status_code}"
        )

        page = body(client.get(WAITING))
        assert "700" not in page, f"чужая сетевая трата видна управляющему:\n{page}"
