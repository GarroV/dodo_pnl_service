...
def test_grid_closed_at_is_english_on_the_english_page(client, clean_closures):
    ...
    #
    # Минуты необязательны: формат `P` при нулевых минутах печатает «6 p.m.»
    # без них. С обязательными минутами тест краснел ровно в первую минуту
    # каждого часа — редкий молчаливый красный, который в CI перезапускают
    # (issue #89, поймано полным прогоном блока auth дважды за вечер).
