...
def test_grid_closed_at_is_english_on_the_english_page(client, clean_closures):
    ...
    #
    # Формат `P` печатает время тремя разными способами, и все три законные:
    # «6:30 p.m.», «6 p.m.» при нулевых минутах и словами — «midnight» в 00:00,
    # «noon» в 12:00. Каждое сужение этого списка давало редкий молчаливый
    # красный, который в CI просто перезапускают: сначала обязательные минуты
    # роняли тест в первую минуту каждого часа (issue #89), потом отсутствие
    # слов уронило его ровно в полночь UTC — поймано полным прогоном в 00:00.
    ENGLISH_STAMP = (
        r"[A-Za-z]{3}\.?\s+\d{1,2},\s+\d{4},\s+"
        r"(\d{1,2}(:\d{2})?\s*[ap]\.m\.|midnight|noon)"
    )
