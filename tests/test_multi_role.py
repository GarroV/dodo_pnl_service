...
def test_without_the_second_role_the_directory_is_refused(client, web_env):
    ...
    Роль здесь управляющего точки: бухгалтер справочники ведёт сам с 28.08.2026
    (D059), и проверять на нём «второй роли нет — доступа нет» стало нечем.
    Смысл проверки от этого не изменился: она страхует механизм нескольких
    ролей от того, чтобы он раздавал права, которых человеку не выдавали.
    """
