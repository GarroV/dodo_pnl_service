...
Чего этот тест НЕ требует: чтобы каждый пункт вёл в написанный экран. С D064
пункты эталона стоят в шапке ВСЕ, и ненаписанные ведут на страницу, которая
называет своё состояние словами («Разработка в процессе»). Прежде их не рисовали
вовсе — и владелец, открыв демо, решил, что продукта нет там, где он есть
(#218). Требование теста другое и прежнее — **у каждого нашего раздела имя из
словаря эталона**, а не своё.
"""
...
def reference_words() -> set[str]:
    ...
    for chunk in (block.group(1) if block else "", menu.group(1) if menu else ""):
        # Не только кириллица: в словаре эталона есть «P&L», и regex, ищущий
        # русскую заглавную, молча пропускал его — а тест из-за этого объявлял
        # правильное название чужим (поймано при добавлении области «Отчёты»).
        words.update(re.findall(r'"([А-ЯЁA-Z][^"]{2,40})"', chunk))
    return words
...
def test_every_section_of_the_product_is_named_by_the_reference():
    ...
    # `str(...)` обязателен и в списке: названия — `gettext_lazy`, и `join`
    # по ним падал `TypeError` вместо того, чтобы назвать виновника. Тест,
    # который ломается вместо того, чтобы объяснить, стоит дороже, чем
    # отсутствующий: он отправляет читать себя, а не свой предмет.
    strangers = [
