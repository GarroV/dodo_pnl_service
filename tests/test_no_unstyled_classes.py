...
def test_no_comment_is_opened_with_a_tag_that_cannot_span_lines():
    """`{# … #}` многострочным не бывает — незакрытый уезжает на экран текстом.

    Django разбирает `{# … #}` в пределах ОДНОЙ строки. Растянутый на несколько
    он не вырезается вовсе: человек видит в ячейке таблицы буквальное
    «# Точка обязательна для ролей… #}». Исходник при этом выглядит нормально,
    и глазами это ловится только на живом экране — поймал смоук, а до него
    комментарий доехал до разметки.

    Сторож здесь, а не в проверке шаблонов вообще, по родству: это тот же класс,
    что «класс поставлен, а правила нет», — в исходнике всё хорошо, а на экране
    мусор. Оговорка уже была записана в `base.html`, и её всё равно нарушили;
    записанное правило без проверки живёт до первого невнимательного.

    Многострочный комментарий пишется `{% comment %} … {% endcomment %}`.
    """
    leaked: list[str] = []
    for template in TEMPLATES:
        for number, line in enumerate(
            ...
            if "{#" in line and "#}" not in line.split("{#", 1)[1]:
                leaked.append(f"{template.relative_to(ROOT)}:{number}: {line.strip()[:60]}")

    assert not leaked, (
        "комментарий `{# … #}` открыт и не закрыт в той же строке — "
        "Django его не вырежет, и он попадёт на экран текстом. "
        "Многострочный комментарий пишется `{% comment %} … {% endcomment %}`:\n"
        + "\n".join(leaked)
    )
