...
def test_states_are_from_the_dictionary():
    ...
    не решает.
    """
    # Слова — те, которыми состояние называет владелец (D064): «там где что-т
    # оесть и мы дорабатываем - там доработка. там где ничего нет - разработка
    # в процессе». Прежнее «беднее» убрано из лексикона тем же решением.
    allowed = {
        "готово", "доработка", "разработка в процессе", "справочная", "Состояние",
    }
...
# статусами — и диспетчер `fabrica`, который читает именно граф, увидел
