...
# «тестов» или «теста» — по-русски это зависит от последней цифры числа, а
# число меняется с каждым добавленным тестом. Жёсткое «тестов» заставляло бы
# писать в README «1542 зелёных тестов» — и тест падал на любом числе,
# кончающемся на 2, 3 или 4, требуя ошибки в грамматике ради зелёного прогона.
PROMISE = re.compile(
    r"Ожидаемый результат — (\d+) зелёных тест(?:ов|а) и (\d+) пропущенных"
)
