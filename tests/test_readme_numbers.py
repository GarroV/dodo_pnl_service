...
# «тестов», «теста» или «тест» — по-русски это зависит от последней цифры
# числа, а число меняется с каждым добавленным тестом. Жёсткая форма заставляла
# бы писать в README неграмотно ради зелёного прогона. Первая редакция принимала
# две формы из трёх и потому падала на числах, кончающихся на 1: «1771 зелёный
# тест» — правильный русский, которого проверка не понимала.
PROMISE = re.compile(
    r"Ожидаемый результат — (\d+) (?:зелёных тест(?:ов|а)|зелёный тест) "
    r"и (\d+) пропущенных"
)
