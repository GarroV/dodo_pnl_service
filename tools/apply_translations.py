...
## Формы множественного числа — руками, инструмент их не трогает

Строку из `{% blocktranslate count %}` инструмент **отказывается** заполнять и
говорит об этом вслух. У такой записи не один перевод, а по форме на каждое
число, и лежат они не в `msgstr`, а в нумерованных `msgstr[N]`:

    #: web/templates/web/cash/expenses.html:161
    #, python-format
...
Сколько форм нужно, объявлено в шапке самого каталога полем `Plural-Forms`:
у английского `nplurals=2`, у сербского `nplurals=3` — и заполнить обязательно
**все**, иначе на числе, которому досталась пустая форма, человек увидит пустоту
вместо строки. Форм меньше объявленного `compilemessages` не замечает; ловит это
`msgfmt --check`, то есть тест `test_the_catalog_survives_a_strict_compile`.

Почему инструмент за это не берётся. Формат входа у него — «одна строка → один
перевод», а выбрать, какой из трёх сербских форм этот перевод предназначался,
можно только угадав. Угадывание переводов в этом проекте уже стоило экрана с
`not calculated yet` над полной ведомостью (T102).
"""
...
def apply(po: Path, table: dict[str, str]) -> tuple[int, list[str]]:
    ...
    while index < len(lines):
        ...
        # Признак записи с формами множественного числа: `msgid_plural` стоит
        # ровно здесь, между ключом и переводом. Отказ — ниже, после того как
        # заведены обе функции про пометки: вернуть их придётся и в этом случае.
        has_plural = index < len(lines) and lines[index].startswith("msgid_plural")
        ...
        # Запись с формами множественного числа не заполняется вовсе.
        #
        # У неё вместо одного перевода — по форме на каждое число, и лежат они
        # в `msgstr[0]`, `msgstr[1]`, … Инструмент искал строку `msgstr` сразу
        # за `msgid`, у такой записи её не находил и дописывал свою — получалось
        # `msgstr` и `msgid_plural` в одной записи, а это для gettext
        # синтаксическая ошибка: каталог не собирается целиком (issue #195).
        #
        # Хуже поломанной сборки была тихая часть: заодно снималась пометка
        # `fuzzy`, и формы, которые `makemessages` перенёс с похожей строки,
        # становились «проверенным переводом». Инструмент при этом печатал
        # «заполнено» и возвращал ноль.
        #
        # Угадать, какой из форм предназначался одиночный перевод, инструмент не
        # вправе: в сербском их три, и правило выбора у каждого языка своё.
        # Поэтому — отказ вслух, а формы вносятся руками (формат в шапке файла).
        if has_plural:
            if source and table.get(source):
                problems.append(
                    "формы множественного числа вносятся руками, "
                    f"одиночный перевод сюда не кладётся: {source[:60]}"
                )
            keep_markers()
            out.extend(old)
            continue
