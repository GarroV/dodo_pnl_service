...
// Роли с НЕПОЛНЫМ набором регистров маршрут отвечает 404 — той же, что на
// случайный адрес (T101). Не 403 и не объяснение: разный ответ на существующую
// и несуществующую строку и есть оракул существования, ради закрытия которого
// проверка роли вынесена перед поиском строки. Словами причина сказана там, где
// человек её ждёт, — на странице периода вместо кнопки (проверено выше).
const hidden = await post(managerRowAction, { reason: REASON });
check("управляющему строка неотличима от несуществующей (404)", hidden.status === 404,
  String(hidden.status));
check("404 ничего не рассказывает о строке", !hidden.text.includes("не входит в права"));

// Отказ по ПРАВУ, словами, проверяется на роли, которая регистры видит все, а
// права морозить не имеет, — администратор сети. До D036 на этом месте стоял
// управляющий, и отказ ему приходил не по праву, а по неполному набору
// регистров: проверка называлась «без права», а доказывала другое.
await login("admin");
await goto(APP + periodHref);
...
// Бухгалтер после D036 видит ровно то же, что директор: равный доступ — это
// проверяемое свойство, а не отсутствие проверки. Числа поэтому директорские.
await login("accountant");
await goto(APP + periodHref);
page = await snapshot();
check("бухгалтер вправе морозить строку", page.freezeForms > 0, String(page.freezeForms));
check("бухгалтер: 60 строк, как у директора", page.count === 60, String(page.count));
check("бухгалтер: итог 1 951 806,13", page.total.trim() === REFERENCE_TOTAL, page.total.trim());
