 ...
 *
 * Три роли, потому что набор областей у них разный: у управляющего нет
 * «Настроек» вовсе, у администратора сети — все три области.
 *
...
await visit("/periods/");
{
  ...
  check("director: области учёта на месте",
    seen.areas.map((a) => a.title).join(" · ") === "Сбор данных · Справочники",
...
{
  const seen = await nav();
  check("admin: три области учёта",
    seen.areas.map((a) => a.title).join(" · ") === "Сбор данных · Справочники · Настройки",
    seen.areas.map((a) => a.title).join(" · "));
  ...
  check("admin: роли и права — в настройках",
    area(seen, "Настройки").items.map((i) => i.href).join(" ") === "/roles/",
